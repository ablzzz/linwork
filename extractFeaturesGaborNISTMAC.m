function extractFeaturesGabor(folder,dest_folder, ext, endian)
%  maxNumCompThreads(1);
...
%   path(path,'/home/kpatil/Timbre/AuditoryToolbox');
  ...
  gab_filters=create_filters_mac(paras,rv,sv);
  mkdir(dest_folder);
...
%    fs = 16000;
	if (length(x)>0.1*fs)
	  ...
	  for i= 1: size(x,1)
	    ...
	    save([dest_filename(1:end-3) '_' num2str(i) '.gabmatlowratemac'], 'crtemp1')
