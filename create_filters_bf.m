...
function gab_filter=create_filters_bf(paras,rv,sv, bw, wt)
...
for j1 = 1 : length(sv)
    for k = 1 : length(rv)
        for find = 1 : length(f0)
            tsf=100;
            fsf=128/5.3;
            G = gaborFilter_MAC(rv(k), sv(j1), 0, tsf , fsf, f0(find), bw(find), wt(find));
            gab_filter(j1, k ,find).G=G;
            G = gaborFilter_MAC(rv(k), sv(j1), 0, tsf , fsf, 5.3 - f0(find), bw(find), wt(find));
...
function [G,tidx,fidx] = gaborFilter_MAC(rateVal, scaleVal, psi, fs_t, fs_f, f0, bw, wt)
